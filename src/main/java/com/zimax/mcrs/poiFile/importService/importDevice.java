...
import com.zimax.mcrs.poiFile.ChangeUtil;
...
public class importDevice {
    ...
    private static ExcelTemplate getTemplateFile(String templateName)
        ...
        //路径
        String templateDir = "F:/zimaxSpace/mcrs/src/main/java/com/zimax/mcrs/poiFile/xls";
...
class ExcelTemplate {
    ...
    private final String TYPE = "com.zimax.mes.sale.orderInfo.Order";
    ...
    public void generateContentDatas(List<DataObject> resultset) {
        ...
        for (Iterator<DataObject> it = resultset.iterator(); it.hasNext(); autoRowId++) {
            DataObject content = it.next();
            startRow++;
    ...
    public int importDictData(String excelFile, int submitCount)
        ...
        for (int sheetCount = 0; sheetCount < wb.getNumberOfSheets(); sheetCount++) {
            ...
            List<Device> typelist = new ArrayList<Device>();
            Set<String> typeset = new HashSet<String>();
            // 从以#开头的的字段名行开始
            int cellno = getDictCellNo(this.fieldNames);
            for (int rowCount = startRow; rowCount < rows; rowCount++) {
                Device[] objs = getDictLine(srcsheet.getRow(rowCount),
                        cellno);

                Device order = new Device();
                String dicttype = String.valueOf(order.getOrderId());
                if (objs.length > 0) {
                    objs[0].set("orderId",dicttype);
                    objs[0].set("sources","ERP导入");
                    objs[0].set("orderStatus","101");
                    objs[0].set("createDate",new Date());
                    objs[0].set("creater/createrId",DataContextManager.current().getMUODataContext().getUserObject()
                            .getUserId());
     ...
     * @return
     */
    private Device[] getDictLine(HSSFRow sourceRow, int cellno) {
        if (sourceRow != null) {
            Device importType = new Device();
            ...
            for (int cellCount = 0; cellCount < fieldNames.length; cellCount++) {
                ...
                if ("materialId".equals(propertyName)) {
                    //propertyName = "eosDictType/materialId";
                }
                //setDataValue(importEntity, propertyName, cell);
            }
            return new Device[] { importType };
        } else {
            return new Device[0];
        }
    }
     ...
     * @return
     */
    private boolean setDataValue(Device importEntity, String propertyName,
                                 HSSFCell cell) {
        if (cell == null || cell.getCellType() == HSSFCell.CELL_TYPE_BLANK) {
            ...
            if (names.length == 2) {
//                Property property = importEntity.
//
//                        .getProperty(names[0]);
                boolean isExistField = isExistField(names[0],importEntity);
                // 如果没有这个属性，直接返回。
//                if (property == null) {
//                    return false;
//                }
                ...
                typeReference = names[1];
//                        (TypeReference) property.getType().getProperty(names[1]).getType();
            }
...
//                        .getProperty(names[1]).getType();
//            } else if (names.length == 1) {
//                if (propertyName.startsWith("type")
//                        && "EosDictType".equals(importEntity.getType()
//                        .getName())) {
//                    propertyName = propertyName.replace("type", "")
//                            .toLowerCase();
//                }
//                if (propertyName.startsWith("entry")
//                        && "EosDictEntry".equals(importEntity.getType()
//                        .getName())) {
//                    propertyName = propertyName.replace("entry", "")
//                            .toLowerCase();
//                }
//
//                Property property = importEntity.getType().getProperty(
//                        propertyName);
//                if (property == null) {
//                    return false;
//                }
...
//                importEntity
//                        .set(propertyName, ChangeUtil.changeToDBDate(value));
//
