...
<script type="text/javascript">
    ...
    var active = {
        ...
        add: function () {
            top.layer.open({
                type: 2,
                title: "添加角色",
                content: "<%= request.getContextPath() %>/basic/aboutEmployee/role/role_add.jsp",
                area: ["800px", "500px"],
                resize: false,
                btn: ["确定", "取消"],
    ...
    table.render({
        ...
        url: "<%=request.getContextPath() %>/EmpRoleController/query",
        method: "get",
        height: "full-" + getFullSize(),
        page: true,
        limit: 10,
    ...
    table.on("tool(LAY-app-user-list)", function (e) {
        ...
        if (e.event == "edit") {
            top.layer.open({
                type: 2,
                title: "编辑用户",
                content: "<%= request.getContextPath() %>/basic/aboutEmployee/role/role_update.jsp",
                area: ["800px", "500px"],
                resize: false,
                btn: ["确定", "取消"],
                success: function (layero, index) {
            ...
            layer.confirm("确定删除该角色？", {
                ...
                roleId = data.roleId;
                $.ajax({
                    url: "<%= request.getContextPath() %>/EmpRoleController/delete?roleId="+ roleId,
                    type: "DELETE",
