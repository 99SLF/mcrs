...
<script type="text/javascript">
    ...
    //判断字符
    form.verify({
        checkJobNo: function (value, item) {
            if (!new RegExp("^[a-zA-Z0-9_]+$").test(value)) {
                return "输入的工号有误，只能输入英文字母、数字、下划线";
            }
            if (value.length > 20) {
                return "输入的工号有误，不能超过20个字符";
            }
            var checkResult = "";
            var flag = "1";
            $.ajax({
                url: "<%=request.getContextPath()%>/EmpController/isExist?jobNo=" + value,
                type: "GET",
                // data: json,
                // data: userId,
                async: false,
    ...
    form.on("submit(layuiadmin-app-form-submit)", function (data) {
        ...
        if (submit == false) {
            ...
            if (isExist == false) {
                $.ajax({
                    url: "<%=request.getContextPath()%>/EmpController/add",
                    type: "POST",
                    data: submitData,
                    cache: false,
                    contentType: "text/json",
